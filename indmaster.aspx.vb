...
Partial Class indmaster
    ...
    Protected Sub BindSelTargets()
        Try
            ...
            strSQL = " SELECT indid, industry, DATE_FORMAT(ST.updatedts, '%m/%d/%Y') AS CreatedDate, U1.UserName as CreatedBy " & _
                    " FROM tblindustry ST " & _
            ...
            strSQL = strSQL & " ORDER BY industry "
    ...
    Protected Sub rptMR_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.RepeaterCommandEventArgs) Handles rptMR.ItemCommand
        Try
            ...
            ElseIf e.CommandName = "Del" Then
                ...
                parms(0) = New DBHelperClient.Parameters("p_indid", e.CommandArgument.Split("|")(0))
                strSQL = " Update tblindustry SET isdeleted = '1' ,updatedts = CURRENT_TIMESTAMP " & _
                   " ,updatedby = " & Session("user_id") & " WHERE indid = " & e.CommandArgument.Split("|")(0)
    ...
    Private Sub lnkSaveList_Click(sender As Object, e As EventArgs) Handles lnkSaveList.Click
        Try
            ...
            End If
            parms(1) = New DBHelperClient.Parameters("p_seltargetid", hdnSelTargetID.Value)
            parms(2) = New DBHelperClient.Parameters("p_ind", pMRName.Text)
            parms(3) = New DBHelperClient.Parameters("p_indprev", hdnTier.Value)
            parms(4) = New DBHelperClient.Parameters("p_user_id", Session("user_id"))
            If hdnSelTargetID.Value = 0 Then
                hdnSelTargetID.Value = db.ExecuteNonQuery(CommandType.StoredProcedure, "SP_IUDIndustry", parms)
            Else
                db.ExecuteNonQuery(CommandType.StoredProcedure, "SP_IUDIndustry", parms)
            End If
