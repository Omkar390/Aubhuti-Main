Imports System.Data
Partial Class accountstatusmaster
    ...
    Protected Sub BindSelTargets()
        Try
            ...
            strSQL = " SELECT atstrowid, stname, U.UserName , DATE_FORMAT(ST.createdts, '%m/%d/%Y') AS createdts, U1.UserName as createdby " & _
                    " FROM tblaccountstatusmaster ST " & _
    ...
    Protected Sub rptMR_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.RepeaterCommandEventArgs) Handles rptMR.ItemCommand
        Try
            ...
            ElseIf e.CommandName = "Del" Then
                ...
                parms(0) = New DBHelperClient.Parameters("p_atstrowid", e.CommandArgument.Split("|")(0))
                strSQL = " Update tblindustry SET isdeleted = '1' ,updatedts = CURRENT_TIMESTAMP " & _
                   " ,modifiedby = " & Session("user_id") & " WHERE indid = @p_atstrowid " 
    ...
    Private Sub lnkSaveList_Click(sender As Object, e As EventArgs) Handles lnkSaveList.Click
        Try
            ...
            End If
            parms(1) = New DBHelperClient.Parameters("p_atstrowid", hdnSelTargetID.Value)
            parms(2) = New DBHelperClient.Parameters("p_stname", pStName.Text)
            parms(3) = New DBHelperClient.Parameters("p_user_id", Session("user_id"))
            If hdnSelTargetID.Value = 0 Then
                hdnSelTargetID.Value = db.ExecuteNonQuery(CommandType.StoredProcedure, "SP_IUDAtStatus", parms)
            Else
                db.ExecuteNonQuery(CommandType.StoredProcedure, "SP_IUDAtStatus", parms)
            End If
